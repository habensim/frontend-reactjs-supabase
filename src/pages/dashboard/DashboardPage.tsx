...
export default function DashboardPage() {
  ...
  const [recentTemplates, setRecentTemplates] = useState<Tables<'templates'>[]>([]);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch user projects instead of templates (since users buy templates, not create them)
      ...
      // Calculate stats based on user's projects and transactions
      ...
      const totalViews = projectsResult.data?.reduce((sum, p) => sum + (p.views || 0), 0) || 0;
      const totalDownloads = projectsResult.data?.reduce((sum, p) => sum + (p.downloads || 0), 0) || 0;

      setStats({
        totalTemplates: totalProjects, // Show user's purchased templates
        totalTransactions,
        totalRevenue,
        totalViews,
        totalDownloads,
        monthlyRevenue: totalRevenue * 0.3, // Mock calculation
        monthlyGrowth: totalProjects > 0 ? 12.5 : 0 // Mock growth percentage
      });
